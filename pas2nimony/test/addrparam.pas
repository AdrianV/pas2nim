program AddrParam;
(* `addr` is an ordinary identifier in Pascal - one corpus unit declares
  `TGetHostByAddr = proc (addr: pointer; len: Integer; ...)` and its
  routines take a parameter of that name. In nimony `addr` is a keyword
  and cannot appear in ANY name position; the generated module reports
  `Error: 'addr' is a keyword and cannot be used as a parameter name`
